...
def apply_weight_sharing(model, bits=5):
    ...
    for name, module in model.named_children():
        ...
        if len(shape) == 2:  # fully connected layers
            ...
            space = np.linspace(min(mat.data), max(mat.data), num=quan_range) # 在mat的最大值與最小值之間產生quan_range個值
            ...
            weight = weight.reshape(shape[0] * shape[1] ,shape[2] * shape[3]) # Q: 如何對應index??
            mat = csr_matrix(weight) if shape[0] < shape[1] else csc_matrix(weight)
