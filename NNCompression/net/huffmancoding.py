    ...
    for name, param in model.named_parameters():
        if 'weight' in name:
            ...
            if 'conv' in name:
                ...
                #################################
                compress_conv = weight.copy().reshape(shape[0] * shape[1] ,shape[2] * shape[3] )
                form = 'csr' if compress_conv.shape[0] < compress_conv.shape[1] else 'csc'
                mat = csr_matrix(compress_conv) if compress_conv.shape[0] < compress_conv.shape[1] else csc_matrix(compress_conv)
                
                # print("mat.indptr.shape" ,mat.indptr.shape)
                # Encoding
                ...
                # print("calc_index_diff(mat.indptr)" , calc_index_diff(mat.indptr))
                # print("calc_index_diff(mat.indptr).shape" ,calc_index_diff(mat.indptr).shape)
    ...
    for name, param in model.named_parameters():
        if 'weight' in name:
            ...
            if 'conv' in name:
                ...
                #################################
                compress_shape = (weight.shape[0] * weight.shape[1] ,weight.shape[2] * weight.shape[3])

                form = 'csr' if compress_shape[0] < compress_shape[1] else 'csc'
                matrix = csr_matrix if compress_shape[0] < compress_shape[1] else csc_matrix
                ...
                # print("huffman_decode" ,huffman_decode(directory, name+f'_{form}_indptr', dtype='int32').shape)
                # print("indptr.shape" ,indptr.shape)

                # Construct matrix
                # print('compress_shape' ,compress_shape)
                mat = matrix((data, indices, indptr), compress_shape)
                mat = mat.toarray().reshape(shape[0] ,shape[1] ,shape[2] ,shape[3])
                ...
                param.data = torch.from_numpy(mat).to(dev)
            elif 'fc' in name:
                form = 'csr' if shape[0] < shape[1] else 'csc'
